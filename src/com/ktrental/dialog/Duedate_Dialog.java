...
//		sqlite.close();
    ...
    private void updateComplete(String aufnr) {
        ...
        contentValues.put("GSUZS", mTime + "00");
                    ...
                    for (BaseMaintenanceModel model : bmm_arr) {
                        if (model.get_gubun().equals("A")) {
                            if (!model.getProgress_status().equals("E0001")) {
                                if (!SELECTED_DAY.substring(0, 6).equals(model.getDay().substring(0, 6))) {
                                    EventPopupC epc = new EventPopupC(context);
                                    epc.show("IoT정비 차량은 생성된 '월' 변경이 불가합니다.");
                                    return;
                                } else {
