...
public class History_Dialog_Transfer_Adapter extends BaseCommonAdapter<HashMap<String, String>> {
	
    private int layout;
	private int checkPosition = -1;
	private ArrayList<HashMap<String, String>> list;
	
	public History_Dialog_Transfer_Adapter(Context context, int layout, ArrayList<HashMap<String, String>> list)
	    {
		super(context);
		mContext = context;
		this.layout = layout;
	 ...
	 public View getView(int position, View convertView, ViewGroup parent)
    	 ...
    	 row1.setText(item.get("ZCODEVT")); //이관상태
    	 row2.setText(item.get("ENAME")); //이관요청자
    	 row3.setText(item.get("REQDT").replace("/", ".")); //이관요청일시
    	 row4.setText(item.get("ENAME2")); //승인/반려자
    	 row5.setText(item.get("RELTM")); //승인/반려일시
