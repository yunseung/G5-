 ...
 * @since 2013.07.12
 */
public class InventoryPopup extends QuickAction implements OnClickListener,
		OnDismissListener {
	...
	public static final int TYPE_MILEAGE = 7; // 거리
	public static final int TYPE_OIL = 8; // 유류량
	private int mType = 0;
	...
	public InventoryPopup(Context context, int orientation, int layoutId,
			int type) {
	...
	public void setInput(String num, boolean delFlag) {
		if (mBtnInput.getText() != null && mType == TYPE_NORMAL_NUMBER) {
			if (mBtnInput.getText().toString().length() >= 10 && !delFlag)
				return;
		}
...
//				else if (mType == TYPE_MILEAGE) {
//					if (mBtnInput.getText().length() > 3) {
//						return;
//					}
//				}
				else {
...
//				num = num.replace(",", "");
//				int int_mile = Integer.parseInt(num);
