 ...
 * @since 2013.07.12
 */
public class InventoryPopup_Dot extends QuickAction implements OnClickListener,
		OnDismissListener {
	...
	public InventoryPopup_Dot(Context context) {
		super(context, QuantityPopup.HORIZONTAL);
		initViewSettings(R.layout.inventory_popup_dot);
		mType = 5;
	}

	public InventoryPopup_Dot(Context context, int nType) {
		super(context, QuantityPopup.HORIZONTAL);
		if(nType==TYPE_NORMAL_NUMBER_UNDER_100)
			initViewSettings_noDot(R.layout.inventory_popup);
		else
			initViewSettings(R.layout.inventory_popup_dot);
		mType = 5;
	}
	...
	private void initViewSettings_noDot(int layoutId) {
		addLayout(layoutId);
		mBtnInput = (TextView) mTrack.findViewById(R.id.inventory_bt_input);
		mBtnInput.setOnClickListener(this);
		mTrack.findViewById(R.id.inventory_bt_input).setOnClickListener(this);
...
//		mTrack.findViewById(R.id.inventory_bt_num_dot).setOnClickListener(this);
	...
	public void setInput(String num, boolean delFlag) {
		...
		if (mBtnInput.getText() != null && mType == TYPE_NORMAL_NUMBER
				&& !delFlag) {
			String strRealNum = mBtnInput.getText().toString() + num;
			int strGetTextLenth = mBtnInput.getText().toString().length();
			int nPos = mBtnInput.getText().toString().indexOf(".");

			if (num.equals(".")) {
				if (mBtnInput.getText().toString().equals(""))
					return;
				if (mBtnInput.getText().toString().contains("."))
					return;
			}
			if (nPos != -1 && strGetTextLenth > nPos + 1)
				return;
			if (Double.valueOf(strRealNum) > 100 && !delFlag)
				return;
		}
